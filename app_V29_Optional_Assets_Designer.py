...
st.set_page_config(page_title="منصة شهادات الخبرة V29 Optional Assets Designer", page_icon="🏆", layout="wide")
...
# ---------------------- أدوات تعديل سريعة في الشريط الجانبي ----------------------
def quick_sidebar_designer(font_labels):
    """
    حل مشكلة أن أدوات التعديل في الأسفل والمعاينة في الأعلى:
    هذه اللوحة تظهر دائماً في الشريط الجانبي، فتستطيع تعديل العنصر بينما المعاينة ظاهرة أمامك.
    """
    st.sidebar.divider()
    st.sidebar.markdown("### 🎨 تعديل سريع")
    st.sidebar.caption("عدّل من هنا وأنت ترى المعاينة بدون النزول للأسفل.")

    keys = list(ELEMENT_NAMES.keys())
    current = st.session_state.get("selected_element", "heading")
    ...
    selected = st.sidebar.selectbox(
        "العنصر",
        keys,
        format_func=lambda k: ELEMENT_NAMES.get(k, k),
        index=keys.index(current),
        key="sidebar_selected_element"
    )
    st.session_state.selected_element = selected

    item = st.session_state.layout.get(selected, {})
    if not item:
        return

    item["visible"] = st.sidebar.checkbox("إظهار العنصر", bool(item.get("visible", True)), key=f"sb_vis_{selected}")

    step = st.sidebar.number_input("خطوة التحريك", 1.0, 50.0, 5.0, 1.0, key="sb_step")

    c1, c2, c3 = st.sidebar.columns(3)
    with c2:
        if st.button("⬆️", key="sb_up", use_container_width=True):
            item["y"] = float(item.get("y", 0)) + step
            st.rerun()
    c1, c2, c3 = st.sidebar.columns(3)
    with c1:
        if st.button("⬅️", key="sb_left", use_container_width=True):
            item["x"] = float(item.get("x", 0)) - step
            st.rerun()
    with c2:
        if st.button("🎯", key="sb_center", use_container_width=True):
            item["x"] = 421
            st.rerun()
    with c3:
        if st.button("➡️", key="sb_right", use_container_width=True):
            item["x"] = float(item.get("x", 0)) + step
            st.rerun()
    c1, c2, c3 = st.sidebar.columns(3)
    with c2:
        if st.button("⬇️", key="sb_down", use_container_width=True):
            item["y"] = float(item.get("y", 0)) - step
            st.rerun()

    # حقول الإحداثيات المباشرة
    xy1, xy2 = st.sidebar.columns(2)
    with xy1:
        item["x"] = st.number_input("X", 0.0, float(PAGE_W), float(item.get("x", 0)), 1.0, key=f"sb_x_{selected}")
    with xy2:
        item["y"] = st.number_input("Y", 0.0, float(PAGE_H), float(item.get("y", 0)), 1.0, key=f"sb_y_{selected}")

    if selected in ["qr", "logo", "stamp", "signature"]:
        s1, s2 = st.sidebar.columns(2)
        with s1:
            item["w"] = st.number_input("العرض", 5.0, 400.0, float(item.get("w", 50)), 1.0, key=f"sb_w_{selected}")
        with s2:
            item["h"] = st.number_input("الارتفاع", 5.0, 400.0, float(item.get("h", 50)), 1.0, key=f"sb_h_{selected}")
    else:
        if item.get("font") not in font_labels:
            item["font"] = font_labels[0] if font_labels else item.get("font", "Arial")

        item["font"] = st.sidebar.selectbox(
            "نوع الخط",
            font_labels,
            index=font_labels.index(item.get("font")) if item.get("font") in font_labels else 0,
            key=f"sb_font_{selected}"
        )
        item["size"] = st.sidebar.number_input("حجم الخط", 6.0, 60.0, float(item.get("size", 12)), 0.5, key=f"sb_size_{selected}")
        item["line_h"] = st.sidebar.number_input("تباعد السطور", 6.0, 50.0, float(item.get("line_h", 16)), 0.5, key=f"sb_lh_{selected}")
        item["w"] = st.sidebar.number_input("عرض الصندوق", 30.0, float(PAGE_W), float(item.get("w", 300)), 5.0, key=f"sb_boxw_{selected}")
        item["chars"] = st.sidebar.number_input("عرض السطر بالحروف", 10, 180, int(item.get("chars", 80)), 1, key=f"sb_chars_{selected}")
        item["bold"] = st.sidebar.checkbox("عريض Bold", bool(item.get("bold", False)), key=f"sb_bold_{selected}")
        item["align"] = st.sidebar.radio(
            "المحاذاة",
            ["right", "center", "left"],
            index=["right", "center", "left"].index(item.get("align", "center")),
            horizontal=True,
            key=f"sb_align_{selected}"
        )
        item["color"] = st.sidebar.color_picker("لون الخط", item.get("color", "#111111"), key=f"sb_color_{selected}")

        if selected in DEFAULT_TEXTS:
            st.sidebar.text_area(
                "نص العنصر",
                value=st.session_state.texts_template.get(selected, ""),
                key=f"sb_text_{selected}",
                height=90
            )
            # مزامنة النص من الشريط الجانبي
            st.session_state.texts_template[selected] = st.session_state.get(f"sb_text_{selected}", st.session_state.texts_template.get(selected, ""))

    st.sidebar.caption("💡 بعد كل تعديل ستُحدّث المعاينة تلقائياً.")


def optional_assets_sidebar_designer():
    """
    لوحة مخصصة للتحكم السريع بالملفات الاختيارية: الشعار، التوقيع، الختم، QR.
    """
    st.sidebar.divider()
    st.sidebar.markdown("### 🖼️ العناصر الاختيارية")
    st.sidebar.caption("تحكم بموضع وحجم الشعار والتوقيع والختم وQR.")
    ...
    selected_asset = st.sidebar.selectbox(
        "اختر العنصر الاختياري",
        list(asset_options.keys()),
        format_func=lambda k: asset_options[k],
        key="optional_asset_selected"
    )

    item = st.session_state.layout.get(selected_asset, {})
    if not item:
        return

    item["visible"] = st.sidebar.checkbox(
        f"إظهار {asset_options[selected_asset]}",
        bool(item.get("visible", False)),
        key=f"asset_vis_{selected_asset}"
    )

    step = st.sidebar.number_input("خطوة تحريك العنصر", 1.0, 50.0, 5.0, 1.0, key="asset_step")

    a, b, c = st.sidebar.columns(3)
    with b:
        if st.button("⬆️", key="asset_up", use_container_width=True):
            item["y"] = float(item.get("y", 0)) + step
            st.rerun()

    a, b, c = st.sidebar.columns(3)
    with a:
        if st.button("⬅️", key="asset_left", use_container_width=True):
            item["x"] = float(item.get("x", 0)) - step
            st.rerun()
    with b:
        if st.button("🎯", key="asset_center", use_container_width=True):
            item["x"] = 421 if selected_asset in ["stamp", "signature"] else item.get("x", 0)
            st.rerun()
    with c:
        if st.button("➡️", key="asset_right", use_container_width=True):
            item["x"] = float(item.get("x", 0)) + step
            st.rerun()

    a, b, c = st.sidebar.columns(3)
    with b:
        if st.button("⬇️", key="asset_down", use_container_width=True):
            item["y"] = float(item.get("y", 0)) - step
            st.rerun()

    st.sidebar.markdown("**الموقع والحجم**")
    p1, p2 = st.sidebar.columns(2)
    with p1:
        item["x"] = st.number_input("X أفقي", 0.0, float(PAGE_W), float(item.get("x", 0)), 1.0, key=f"asset_x_{selected_asset}")
        if "w" in item:
            item["w"] = st.number_input("العرض W", 5.0, 500.0, float(item.get("w", 50)), 1.0, key=f"asset_w_{selected_asset}")
    with p2:
        item["y"] = st.number_input("Y عمودي", 0.0, float(PAGE_H), float(item.get("y", 0)), 1.0, key=f"asset_y_{selected_asset}")
        if "h" in item:
            item["h"] = st.number_input("الارتفاع H", 5.0, 500.0, float(item.get("h", 50)), 1.0, key=f"asset_h_{selected_asset}")

    s1, s2 = st.sidebar.columns(2)
    with s1:
        if st.button("➕ تكبير", key="asset_bigger", use_container_width=True):
            if "w" in item:
                item["w"] = float(item.get("w", 50)) + step
            if "h" in item:
                item["h"] = float(item.get("h", 50)) + step
            st.rerun()
    with s2:
        if st.button("➖ تصغير", key="asset_smaller", use_container_width=True):
            if "w" in item:
                item["w"] = max(5, float(item.get("w", 50)) - step)
            if "h" in item:
                item["h"] = max(5, float(item.get("h", 50)) - step)
            st.rerun()

    st.sidebar.markdown("**أماكن جاهزة**")
    p1, p2 = st.sidebar.columns(2)
    with p1:
        if st.button("أعلى اليمين", key="asset_top_right", use_container_width=True):
            item["x"] = 730
            item["y"] = 455
            if "w" in item:
                item["w"] = 55
            if "h" in item:
                item["h"] = 55
            st.rerun()
        if st.button("أسفل اليسار", key="asset_bottom_left", use_container_width=True):
            item["x"] = 115
            item["y"] = 70
            if "w" in item:
                item["w"] = 50
            ...
            st.rerun()
    with p2:
        if st.button("أسفل الوسط", key="asset_bottom_center", use_container_width=True):
            item["x"] = 390
            item["y"] = 72
            if "w" in item:
                item["w"] = 70
            if "h" in item:
                item["h"] = 45
            st.rerun()
        if st.button("أسفل اليمين", key="asset_bottom_right", use_container_width=True):
            item["x"] = 555
            item["y"] = 72
            if "w" in item:
                item["w"] = 70
    ...
    st.sidebar.caption("💾 عند حفظ القالب سيتم حفظ مواضع هذه العناصر أيضاً.")
...
def main():
    ...
    /* V27: تثبيت عمود المعاينة أثناء النزول في لوحة التعديل */
    @media (min-width: 1000px) {
        div[data-testid="column"]:has(#preview-sticky-anchor) {
            position: sticky;
            top: 12px;
    ...
    st.title("🏆 منصة شهادات الخبرة - V29 Optional Assets Designer")
    ...
    with tab_issue:
        ...
        quick_sidebar_designer(font_labels)
        optional_assets_sidebar_designer()
        bgs = backgrounds()
        ...
        with left:
            ...
            with st.expander("📎 رفع ملفات اختيارية: شعار / ختم / توقيع", expanded=False):
            ...
            st.markdown('<div class="designer-tip">✅ الآن يمكنك تعديل النصوص والعناصر الاختيارية مثل الشعار والتوقيع والختم وQR من الشريط الجانبي الأيسر مباشرة بينما المعاينة ظاهرة أمامك.</div>', unsafe_allow_html=True)
        ...
        with right:
            st.markdown('<div id="preview-sticky-anchor"></div>', unsafe_allow_html=True)
            texts = render_texts(values)
